...
const mainMenuKeyboard = {
  ...
  payload: {
    buttons: [
      [
        { type: 'callback', text: '🔍 Найти маршруты', payload: 'find_routes' }
      ],
      [
      ...
      [
        { type: 'callback', text: '⚙️ Настройки', payload: 'settings' } // ← ДОБАВИЛИ
      ],
      [
...
function getRouteKeyboard(routes) {
    ...
    for (let i = 1; i <= Math.min(routes.length, 5); i++) {
        ...
        if (row.length === 3) {  // максимум 3 кнопки в ряду для callback
            buttons.push([...row]);
            row.length = 0;
        }
    }
...
// Клавиатура для запроса геолокации (правильный формат)
const geoRequestKeyboard = {
    ...
    payload: {
        buttons: [
            [
                { 
                    type: 'request_geo_location',  // ✅ правильный тип для MAX
...
const settingsKeyboard = {
  ...
  payload: {
    buttons: [
      [
        { type: 'request_geo_location', text: '📍 Изменить местоположение', payload: 'change_location' }
      ],
      [
        { type: 'callback', text: '🏠 Главное меню', payload: 'main_menu' }
      ]
